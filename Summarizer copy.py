...
# --- FUNCTION TO GET USER FEEDBACK FOR A FIELD ACROSS ALL DOCUMENTS ---
def get_field_feedback_across_documents(field, document_results, document_texts, filenames):
    """Get user feedback for a specific field and apply it across all documents"""
    print("\n" + "="*80)
    ...
    print("\nOptions:")
    print("1. Accept all current values")
    print("2. Edit values manually for each document")
    ...
    if choice == "1":
        print(f"{Fore.GREEN}All values accepted.{Style.RESET_ALL}")
        # Set confidence to 1.0 for this field in all documents
        for result in updated_results:
            if field in result:
                result[field]["confidence"] = 1.0
            
    elif choice == "2":
        # Allow user to edit each document separately
        for i, (result, filename) in enumerate(zip(updated_results, filenames)):
            field_data = result.get(field, {"value": "Not Found", "confidence": 0.0})
            current_value = field_data["value"]
            new_value = input(f"Enter new value for {field} in document [{i+1}] {filename} (current: '{current_value}'): ").strip()
        ...
        print(f"{Fore.GREEN}All values updated.{Style.RESET_ALL}")
        ...
        print(f"{Fore.RED}Invalid choice. Keeping original values.{Style.RESET_ALL}")
